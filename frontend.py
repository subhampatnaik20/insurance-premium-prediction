...
if st.button("Predict Premium Category"):
    ...
    try:
        ...
        if response.status_code == 200 and "predicted_category" in result:
            # We don't need the 'prediction = result["response"]' line anymore
            
            st.success(f"Predicted Insurance Premium Category: **{result['predicted_category']}**")
            
            # Use .get() to safely check for these, so the app doesn't crash 
            # if your backend isn't sending them yet!
